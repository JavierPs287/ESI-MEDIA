...
class OldVideoControllerTest {
    ...
    @Test
    @DisplayName("Debe fallar cuando resolución es negativa")
    void testUploadVideoNegativeResolution() throws Exception {
        // Arrange
        validVideoDTO.setResolution(-1080);
        when(videoService.uploadVideo(any(VideoDTO.class))).thenThrow(new IllegalArgumentException("La resolución del video debe ser un valor positivo."));
        ...
        mockMvc.perform(post("/creador/uploadVideo")
                ...
                .content(objectMapper.writeValueAsString(validVideoDTO)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error").value("La resolución del video debe ser un valor positivo."));
    }
